 ...
 */
@Ignore
public class NodeTypeTest<T extends Node>
   ...
   *          the node type
   */
  private NodeTypeTest(final _StructureTest<?> owner,
   ...
   *          the node type
   */
  protected NodeTypeTest(final NodeType<T> type) {
    this(null, type);
  }
  ...
  public void testChildTypesRecursively() {
    ...
    if (size > 0) {
      ...
      for (int i = ch.length; i > 0;) {
        ...
        ch[choice] = ch[--i];
        NodeTypeSetTest._testNodeTypeSet(this, sel);
      }
    } else {
    ...
    if (type.isTerminal()) {
      ...
      for (int i = ns.length; (--i) >= 0;) {
        ...
        if ((nts.getTerminalTypeCount() > 0) && (d > 4)) {
          ns[i] = NodeTypeTest._instantiate(
              nts.getRandomTerminalType(r), r, d + 1);
        } else {
          ns[i] = NodeTypeTest._instantiate(nts.getRandomType(r),
              r, d + 1);
        }
      }
  ...
  public void testInstantiate() {
    final Node n = NodeTypeTest._instantiate(this.getInstance(),
        ThreadLocalRandom.current(), 0);
    Assert.assertNotNull(n);
    NodeTest._testNode(this, n);
  }
  ...
  static void _testNodeType(final _StructureTest<?> owner,
    ...
    owner._test(nodeType,
        () -> new NodeTypeTest<>(owner, nodeType).runAllTests());
  }
  ...
  public static void testNodeType(final NodeType<?> nodeType) {
    new NodeTypeTest<>(nodeType).runAllTests();
  }
}
