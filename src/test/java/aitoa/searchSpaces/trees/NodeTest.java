 ...
 */
@Ignore
public class NodeTest extends _StructureTest<Node> {
   ...
   *          the owner
   */
  private NodeTest(final _StructureTest<?> owner,
      final Node node) {
    super(owner);
   ...
   *          the node
   */
  protected NodeTest(final Node node) {
    this(null, node);
  }
  ...
  public void testNodeType() {
    ...
    if (!(NodeTypeTest.isDummyNodeType(t))) {
      NodeTypeTest._testNodeType(this, t);
  ...
  static void _testNode(final _StructureTest<?> owner,
      final Node node) {
    owner._test(node,
        () -> new NodeTest(owner, node).runAllTests());
  }
