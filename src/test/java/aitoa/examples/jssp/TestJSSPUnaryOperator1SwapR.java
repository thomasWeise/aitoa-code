...
/**
 * test the unary 1-swap search operator with random enumeration
 * order
 */
public class TestJSSPUnaryOperator1SwapR
    extends IUnarySearchOperatorTest<int[]> {

  /** the space we use */
  private static final JSSPInstance PROBLEM =
      new JSSPInstance("yn1"); //$NON-NLS-1$

  /** the space we use */
  private static final JSSPSearchSpace SPACE =
      new JSSPSearchSpace(TestJSSPUnaryOperator1SwapR.PROBLEM);

  /** the operator we use */
  private static final IUnarySearchOperator<int[]> OP =
      new JSSPUnaryOperator1SwapR(
          TestJSSPUnaryOperator1SwapR.PROBLEM);

  /** {@inheritDoc} */
  @Override
  protected ISpace<int[]> getSpace() {
    return TestJSSPUnaryOperator1SwapR.SPACE;
  }
  ...
  protected IUnarySearchOperator<int[]>
    ...
    return TestJSSPUnaryOperator1SwapR.OP;
  }
  ...
  protected int[] createValid() {
    return JSSPTestUtils
        .createValidX(TestJSSPUnaryOperator1SwapR.PROBLEM);
  }
