 ...
 * parallelized at all.
 */
public class JSSPExperimentOld {
  ...
  public static final void main(final String[] args)
    ...
    for (final String instId : JSSPExperimentOld.INSTANCES) {
      // load the instance
      final JSSPInstance inst = new JSSPInstance(instId);
      final int upperBound =
          JSSPExperimentOld.__get_makespan_upper_bound(inst);
      boolean plainEDADone = false;

// random samplers
      JSSPExperimentOld.run(new SingleRandomSample<>(), null,
          null, inst, out);
      JSSPExperimentOld.run(new RandomSampling<>(), null, null,
          inst, out);
...
// plain hill climbers which do not enumerate their neighborhood
          JSSPExperimentOld.run(new HillClimber<>(), unary, null,
              inst, out);
...
// hill climbers with neighborhood enumeration
        if (unary.canEnumerate()) {
          JSSPExperimentOld.run(new HillClimber2<>(), unary,
              null, inst, out);
          JSSPExperimentOld.run(new HillClimber2WithRestarts<>(),
              unary, null, inst, out);
...
// simulated annealing
        if (!(unary instanceof JSSPUnaryOperator1SwapR)) {
          ...
          for (final double ep : new double[] { 2e-7d, 4e-7d,
              8e-7d }) {
            JSSPExperimentOld.run(new SimulatedAnnealing<>(
          ...
          for (final int mu : new int[] { 16, 32, 64, 512, 2048,
            ...
            for (final int lambda : new int[] { mu }) {
              for (final double cr : new double[] { 0, 0.05,
                ...
                if (!(unary instanceof JSSPUnaryOperator1SwapR)) {
// the plain EA
                  JSSPExperimentOld.run(new EA<>(cr, mu, lambda),
...
// only do EDA once
                JSSPExperimentOld.run(
                    new EDA<>(mu, lambda, model), null, null,
                    inst, out);
// and the version with fitness
              ...
              if (unary.canEnumerate()) {
                if (!(unary instanceof JSSPUnaryOperator1SwapR)) {
                  for (final int steps : new int[] {
                      Integer.MAX_VALUE, 10, 100 }) {
                    JSSPExperimentOld.run(new HybridEDA<>(mu,
      ...
      for (final int maxDepth : new int[] { 6, 8 }) {
        JSSPExperimentOld.__runGP(new RandomSampling<>(),
            maxDepth, inst, out);
        // evolutionary algorithms
        for (final int mu : new int[] { 16, 128, 1024 }) {
          for (final int lambda : new int[] { mu }) {
            for (final double cr : new double[] { 0.05, 0.3 }) {
              // the plain EA
              JSSPExperimentOld.__runGP(new EA<>(cr, mu, lambda),
                  maxDepth, inst, out);
// the EA with pruning, i.e., which enforces population diversity
...
// set the maximum runtime
    builder.setMaxTime(JSSPExperimentOld.MAX_TIME);
