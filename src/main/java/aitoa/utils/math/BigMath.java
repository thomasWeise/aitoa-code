...
public class BigMath {
   ...
   */
  public static final double ld(final BigInteger val) {
    // Get the minimum number of bits necessary to hold this
    // value.
    final int n = val.bitLength();

    // Calculate the double-precision fraction of this number; as
    // if the
    ...
    // because the
    // number of bits is always higher than necessary for a
    // number
    // (ie. log2(val)<n for every val).
    return ((n - 1) + (Math.log(mantissa / ((double) (1L << 52)))
        * 1.44269504088896340735992468100189213742664595415298d));
    // Magic number converts from base e to base 2 before adding.
    // For other
